...
import hsb.html.select.CombiningEvaluator;
...
public class XPathParser {
        ...
        FUNCTION_MAPPING.put("contains", new FunctionEvaluator() {
            ...
            public Evaluator call(String... param) {
                ...
                if (param[0].equals("id")) {
                    return new Evaluator.IdContain(param[1].getBytes(StandardCharsets.UTF_8));
                } else if (param[0].equals("class")) {

                } else if (param[0].equals("href")) {
    ...
    private void byTag() {
        ...
        evals.add(new Evaluator.Tag(tagName.trim().toLowerCase().getBytes(StandardCharsets.UTF_8)));
    }
    ...
    private Evaluator byAttribute(XTokenQueue cq) {
        ...
        if (cq.isEmpty()) {
            if ("*".equals(key)) {
                ...
                evaluator = new Evaluator.Attribute(keyType);
            }
        } else {
            if (cq.matchChomp("=")) {
                ...
                if (key.equals("id")) {
                    evaluator = new Evaluator.Id(XTokenQueue.trimQuotes(value).getBytes(StandardCharsets.UTF_8));
                } else if (key.equals("class")) {
                    String className = XTokenQueue.trimQuotes(value);
                    if (!className.contains(" ")) {
                        evaluator = new Evaluator.Class(className.getBytes(StandardCharsets.UTF_8));
                    } else {
